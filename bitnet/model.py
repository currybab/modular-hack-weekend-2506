...
def bitnet_int8xint2_linear_mojo(input0, input1, s, ws):
    ...
    out_shape = list(input0.shape)
    out_shape[-1] = input1.shape[0]
    output_tensor = torch.empty(out_shape, dtype=torch.bfloat16, device=input0.device)
    ...
    torch.compile(bitlinear)(output_tensor, input0, input1, s, ws)
